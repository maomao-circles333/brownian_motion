...
# Here we use CBO-style noise, as suggested by Jethro. Namely, the intrinsic mean formula is defined by Ci = argmin sum wij d^2(xi, y) for y over S^{d-1}.
# * Ci is different for each x_i
#
# Plots:
...
os.environ.setdefault("JAX_ENABLE_X64", "0")  # "1" for using float64
...
RUNS       = 100
STORE_STRIDE = 10
MEAN_UPDATE_STRIDE = 5    # for intrinsic mean refinement and C_i recomputation
MEAN_REFINE_STEPS  = 2    # number of Karcher refinements when we do recompute
INIT_SEED  = 201
NOISE_SEED = 999
...
def intrinsic_mean_sd_jax(points, tol=1e-10, max_iter=100):
    ...
    points: (m,d) on S^{d-1}
    Returns u: (d,)
    Same fixed-point iteration as before, just JAX while_loop.
    """
...
@jax.jit
def _karcher_refine_all_C(z, X_in, W_in):
    """
    One Karcher refinement step for C_i across (runs, agents) in parallel.
    z:    (R,n,d) current centers C
    X_in: (R,n,d) points
    W_in: (R,n,n) weights w_{i<-j} per (run, i, j)
    Returns: (R,n,d) refined z
    """
    X_rep = jnp.repeat(X_in[:, None, :, :], X_in.shape[1], axis=1)   # (R,n,n,d)
    z_exp = z[:, :, None, :]                                         # (R,n,1,d)

    c  = jnp.clip(jnp.sum(X_rep * z_exp, axis=-1), -1.0, 1.0)        # (R,n,n)
    th = jnp.arccos(c)                                               # (R,n,n)
    Uperp = X_rep - c[..., None] * z_exp                              # (R,n,n,d)
    su = jnp.linalg.norm(Uperp, axis=-1, keepdims=True)               # (R,n,n,1)
    fac = th[..., None] / jnp.maximum(su, 1e-7)                       # (R,n,n,1)
    fac = jnp.where(th[..., None] < 1e-7, 0.0, fac)
    V = fac * Uperp                                                   # (R,n,n,d)

    g = jnp.einsum('rij,rijd->rid', W_in, V)                          # (R,n,d)

    nv = jnp.linalg.norm(g, axis=-1, keepdims=True)                   # (R,n,1)
    y  = jnp.cos(nv) * z + jnp.sin(nv) * (g / jnp.maximum(nv, 1e-7))
    y_small = norm_last(z + g)
    z_next  = jnp.where(nv < 1e-7, y_small, y)
    return norm_last(z_next)

def compute_Ci_from_X(X, beta, mean_refine_steps):
    inner_ij = X @ jnp.swapaxes(X, -1, -2)     # (R,n,n)
    w_ij     = softmax_last(beta * inner_ij)   # (R,n,n)
    C        = norm_last(jnp.einsum('rij,rjd->rid', w_ij, X))  # (R,n,d)
    # fixed amount of small refinement
    for _ in range(int(mean_refine_steps)):
        C = _karcher_refine_all_C(C, X, w_ij)  
    return C
...
def simulate_intrinsic_noise_jax(n, d, T, dt, b, sigma0,
    ...
    # Strong solve (accurate) for the initial run-wise intrinsic mean
    U0 = intrinsic_mean_sd_batch(X0, 1e-10, 100)  # (R,d)

    C_cache0 = compute_Ci_from_X(X0, jnp.float32(b), mean_refine_steps)  # (R,n,d)
    ...
    def run():
        def step_fn(carry, k):
            ...
            def recompute_C(_):
                return compute_Ci_from_X(X, b32, mean_refine_steps)
            do_update_C = (k % MEAN_UPDATE_STRIDE) == 0
            C_used = lax.cond(do_update_C, recompute_C, lambda _: C_cache, operand=None)
            ...
            # ===== per-agent weighted Karcher mean C_i =====
            c_i = jnp.clip(jnp.sum(X * C_used, axis=-1, keepdims=True), -1.0, 1.0)  # (R,n,1)
...
def main():
    ...
    # Noisy runs (shared init condition)
    ...
    # --- Per-agent mean trajectories (vectorized JAX intrinsic means) ---
    M_agents = np.array(mean_trajectories_per_agent_jax(traj_noisy))  # (kept, n, d)

    # --- Drift vs deterministic direction (for print) ---
    ...
    if DIM == 3:
        ...
        # Global PCA on all agent-mean points
