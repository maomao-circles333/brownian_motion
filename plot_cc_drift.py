...
def parse_args():
    ...
    ap.add_argument("--color", type=str, default="#6f42c1", help="Purple color for band/median")
...
def aggregate_per_sigma(args):
    ...
    for f in files:
        ...
        # sigma consistency later; we collect then check
        per_sigma[sidx]["sigma"].append(rec["sigma"])
        # merge by init index (last write wins), but here we just collect and dedup below
        per_sigma[sidx]["geo_rad_vals"].append((rec["init_indices"], rec["drifts"]))

    # consolidate maps and check sigma
    for sidx, blob in per_sigma.items():
        sigs = [s for s in blob["sigma"] if not np.isnan(s)]
        if sigs and (np.max(sigs) - np.min(sigs) > 1e-9):
            raise SystemExit(f"[ERROR] sigma_idx={sidx:03d}: mixed sigma values; clean OUTDIR/JOBNAME")
...
def make_series(per_sigma, mode):
    ...
    for sidx in sorted(per_sigma.keys()):
        ...
        # --- Geodesic in degrees ---
        deg = np.rad2deg(rad)  # convert each sample, THEN compute band
        ...
        euc = 2.0 * np.sin(0.5 * rad)  # unit sphere chord length
...
def main():
    ...
    csv_geo = os.path.join(args.outdir, f"{args.plot_prefix}_geodesic_deg.csv")
    csv_euc = os.path.join(args.outdir, f"{args.plot_prefix}_euclidean.csv")
    ...
    # Geodesic (degrees)
    png_g = os.path.join(args.outdir, f"{args.plot_prefix}_geodesic_deg_dt1.png")
    pdf_g = os.path.join(args.outdir, f"{args.plot_prefix}_geodesic_deg.pdf")
    plot_band(
    ...
    png_e = os.path.join(args.outdir, f"{args.plot_prefix}_euclidean_dt1.png")
    pdf_e = os.path.join(args.outdir, f"{args.plot_prefix}_euclidean.pdf")
    plot_band(
